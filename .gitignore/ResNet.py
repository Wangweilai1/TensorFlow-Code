...
class Block(collections.namedtuple('Block', ['scope', 'unit_fn', 'args'])):
	...
	def conv2d_same(inputs, num_outputs, kernel_size, stride, scope = None):
		if stride == 1:
			...
			return slim.conv2d(inputs, num_outputs, kernel_size, stride = stride, padding = 'VAILD', scope = scope)
	@slim.add_arg_scope
	def block in blocks:
	...
	def resnet_arg_scope(is_training = True, weight_decay = 0.0001, batch_norm_decay = 0.997,
						bathc_norm_epsilon = 1e-5, batch_norm_scale = True):
		batch_norm_params = { 
			'is_training': is_training,
			'decay':	batch_norm_decay,
			'epsilon': bathc_norm_epsilon,
		...
		with slim.arg_scope([slim.conv2d], weights_regularizer = slim.l2_regularizer(weight_decay),
				...
				with slim.arg.arg_scope([slim.batch_norm], **batch_norm_params):
	...
	def bottleneck(inputs, depth, depth_bottleneck, stride, outputs_collections = None, scope = None):
		with tf.variable_scope(scope, 'bottleneck_v2', [inputs]) as sc:
			...
			return slim.utils.collect_name_outputs(outputs_collections, sc.name, output)
	def resnet_v2(inputs, blocks, num_classes = None, global_pool = True, include_root_block = True,
		...
		with tf.variable_scope(scope, 'resnet_v2', [inputs], reuse = reuse) as sc:
			...
			with slim.arg_scope([slim.conv2d, bottleneck, stack_blocks_dense], outpus_collections = end_points_collection):
				net = inputs
				if include_root_block:
					with slim.arg_scope([slim.conv2d], activation_fn = None, normalizer_fn = None):
						net = conv2d_same(net, 64, 7, stride = 2, scope = 'conv1')
					net = slim.max_pool2d(net, [3, 3], stride = 2, scope = 'postnorm')
